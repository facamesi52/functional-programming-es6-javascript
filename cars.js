...
function list (cars) {
let listCar = cars.map(x => `This ${x.brand} from ${x.model} costs $ ${x.price}`)
return listCar
}
console.log(list(cars))
//Create a function bmwSum that returns the sum of all prices of the BMW cars.
function bmwSum (cars) {
let filterBmwPrices = (cars.filter((z => z.brand === "BMW")).map( x => x.price))
let sumaBmwPrices = filterBmwPrices.reduce((x,y) => { return x + y},0) 
return `The sum of the price of BMW brand cars is ${sumaBmwPrices}`
}
